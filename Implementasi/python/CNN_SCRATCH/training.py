...
def train(num_iteration):
    ...
    for i in range(total_iterations,total_iterations + num_iteration):
        ...
        if i % int(data.train.num_examples/batch_size) == 0:
            ...
            save_path = saver.save(session, "model/cnn_model.ckpt")
...
validation_data = data.valid.img_names
validation_labels = data.valid.labels
...
test_features_cnn = np.zeros((VALIDATION_SIZE, NUMBER_OF_FEATURES), dtype=float)
test_labels_cnn = np.zeros(VALIDATION_SIZE, dtype=int)


for i in range(0,num_iter):
    train_batch, y_true_batch, _, cls_batch = data.train.next_batch(BATCH_SIZE)
    features_batch = layer_fc1.eval(feed_dict={x: train_batch}, session=session)
    for j in range(BATCH_SIZE):
        for k in range(NUMBER_OF_FEATURES):
            train_features_cnn[BATCH_SIZE * i + j, k] = features_batch[j, k]
            train_labels_cnn[BATCH_SIZE * i + j] = np.sum(np.multiply(converter, y_true_batch[j, :]))
