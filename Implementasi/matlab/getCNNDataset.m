function [imdstraining, imdstest] = getCNNDataset(dirlist,doFaceDetection,size)
...
imdstraining.ReadFcn = @(filename)readAndPreprocessCNNImage(filename,doFaceDetection,size);
imdstest.ReadFcn = @(filename)readAndPreprocessCNNImage(filename,doFaceDetection,size);
end
